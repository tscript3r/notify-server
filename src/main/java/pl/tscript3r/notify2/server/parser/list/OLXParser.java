...
import pl.tscript3r.notify2.server.factory.AdFactory;
...
public class OLXParser extends JsoupParser {
	...
	private void mainLayoutParser(Elements adsElements) {
		adsElements.forEach(element -> {
			...
			ad.addValue(AdFields.KEY_ID, new String(element.attr("data-id")));
			ad.addValue(AdFields.KEY_URL, adUrl);
			ad.addValue(AdFields.KEY_TITLE, element.select("strong").text());
			ad.addValue(AdFields.KEY_THUMBNAIL_URL, element.select("img[src]").attr("src"));
			ad.addValue(AdFields.KEY_CATEGORY, element.select("small[class]").text());
			ad.addValue(AdFields.KEY_PRICE, element.select("p").select("strong").text());
			ad.addValue(AdFields.KEY_LOCATION,
					element.select("small[class]").attr("class", "breadcrumb x-normal").select("span").text());
			sendToQueue(ad);
		});
	}
	...
	private void workLayoutParser(Elements adsElements) throws ParseException {
		adsElements.forEach(element -> {
			if (StringUtils.containsIgnoreCase(element.attr("class"), "offer ")) {
				AdPackage ad;
				String adUrl = element.select("a[href]").attr("href");
				ad = AdFactory.getAdPackageInstance(adUrl, recipient);
				String title = element.select("div[class]").attr("class", "list-item__price").text();
				ad.addValue(AdFields.KEY_URL, adUrl);
				ad.addValue(AdFields.KEY_TITLE, title);
				ad.addValue(AdFields.KEY_ID, new String(longHash(title)));
				ad.addValue(AdFields.KEY_LOCATION,
						element.select("strong[class]").attr("strong", "list-item__location").text());
				sendToQueue(ad);
			}
		});
	}
	...
	public int parse() throws ParseException, IOException {
		parsedContent = Jsoup.connect(url).userAgent("Mozilla/5.0 Chrome/26.0.1410.64 Safari/537.31")
				  					 	  .timeout(5*1000)
		...
		if (adsElements.size() > 0)
			...
			adsElements = parsedContent.getElementsByAttributeValueMatching("class", Pattern.compile("offer\\s{0,8}"));
			if (adsElements.size() > 0)
				workLayoutParser(adsElements);
			else
				throw new ParseException("OLX.pl site has been changed - parser needs to be rebuild", 0);
		}

		if (firstRun)
			firstRun = false;
		return adsElements.size();
	}
	...
	public boolean equals(Object obj) {
		if (obj != null)
			return (obj.hashCode() == taskId) ? true : false;
		return false;
	}
